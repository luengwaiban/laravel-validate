...
class TagController extends BaseController{
    ...
    public function index(){
        $this->route = 'tags';
        $title = '标签管理';
        $filter = DataFilter::source(Tag::rapydGrid());
        $filter->add('id','标签ID','text')
            ->scope(function($query,$value){
                return $value?$query->where('t.id',$value):$query;
            });
        $filter->add('status','审核状态','select')->options([''=>'全部状态']+self::$statusText)
                ->scope(function($query,$value){
                    return ($value!==null&&$value!=='')?$query->where('t.status',$value):$query;
                });
        $filter->submit('筛选');
        $filter->reset('重置');
        $filter->build();
        ...
        $grid->add('name','标签名称',false);
        $grid->add('user_name','创建标签用户',false);
        $grid->add('status', '审核状态', false);
        $grid->add('created_at', '创建时间', true);
        $grid->add('operation','操作',false);

        $grid->orderBy('id', 'desc');
        $grid->link('/'.config('admin.route.prefix') . '/tags/edit', '添加', 'TR', ['class' => 'btn btn-primary']);
        $grid->row(function($row){
            $link = '/'.config('admin.route.prefix') . '/tags/edit?modify='.$row->data->id;
    ...
    public function edit(){
        ...
        if($change = Input::get('change',null) ){
            ...
            if($id)
            Tag::where('id',$id)->update(['status'=>$change]);
        ...
        $edit->add('name','标签名称','text')->rule('required');
